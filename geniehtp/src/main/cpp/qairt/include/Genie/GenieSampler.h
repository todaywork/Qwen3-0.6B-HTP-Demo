...
/**
 * @brief A callback invoked by GenieSampler_sampleData() to deliver sampled tokens.
 *
 * @param[in] numTokens Number of tokens returned.
 *
 * @param[in] tokens Pointer to tokens array of length numTokens.
 *
 * @param[in] userData User provided pointer forwarded by GenieSampler_sampleData().
 */
typedef void (*GenieSampler_Callback_t)(const uint32_t numTokens,
...
/**
 * @brief A function to create a sampler instance from a sampler configuration.
 *
 * @param[in]  configHandle   A sampler config handle.
 *
 * @param[out] samplerHandle  A handle to the created sampler. Must not be NULL.
 *
 * @return Status code:
 *         - GENIE_STATUS_SUCCESS: API call was successful.
 *         - GENIE_STATUS_ERROR_INVALID_HANDLE: Sampler config handle is invalid.
 *         - GENIE_STATUS_ERROR_INVALID_ARGUMENT: At least one argument is invalid.
 *         - GENIE_STATUS_ERROR_MEM_ALLOC: Memory allocation failure.
 *         - GENIE_STATUS_ERROR_GENERAL: Other failure.
 */
GENIE_API
...
/**
 * @brief A function to free a sampler instance.
 *
...
/**
 * @brief Sample tokens from a logits buffer using the sampler configuration already applied to
 *        samplerHandle.
 *
 * @param[in] samplerHandle A sampler handle.
 *
 ...
 *
 * @param[in] dataConfig JSON string describing the logits tensor (e.g. { "dimensions":[...],
 *                       "data-type":"float" }).
 *
 ...
 *         - GENIE_STATUS_ERROR_INVALID_HANDLE: Sampler handle is invalid.
 *         - GENIE_STATUS_ERROR_INVALID_ARGUMENT: At least one argument is invalid.
 *         - GENIE_STATUS_ERROR_JSON_FORMAT: dataConfig is not valid JSON.
 *         - GENIE_STATUS_ERROR_JSON_SCHEMA: dataConfig is missing required fields.
 *         - GENIE_STATUS_ERROR_GENERAL: Other failure.
 */
GENIE_API
